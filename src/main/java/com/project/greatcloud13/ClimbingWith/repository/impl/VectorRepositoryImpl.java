...
//      3. 검색된 텍스트 조각들을 리스트로 변환
    ...
    @Override
    public List<TextSegment> searchByGym(String query, Long gymId, int maxResults) {

//      1. 검색 질문 벡터로 변환 (임베딩 모델과 같은 모델을 사용해야한다.)
        Embedding queryEmbedding = embeddingModel.embed(query).content();
//      2. 암장 필터 설정
        Filter gymFilter = MetadataFilterBuilder.metadataKey("gymId").isEqualTo(gymId);

//      3. 유사도 검색 실행
//      minScore를 통해 너무 관련 없는 결과는 제거하도록 함
//      필터 추가를 통해 특정 암장의 게시글에서만 검색
        EmbeddingSearchRequest request = EmbeddingSearchRequest.builder()
                .queryEmbedding(queryEmbedding)
                .maxResults(maxResults)
                .filter(gymFilter)
...
//      3. 검색된 텍스트 조각들을 리스트로 변환
    ...
    @Override
    public List<TextSegment> searchByPostType(String query, PostType postType, int maxResults) {

//      1. 검색 질문 벡터로 변환 (임베딩 모델과 같은 모델을 사용해야한다.)
        Embedding queryEmbedding = embeddingModel.embed(query).content();
//      2. 암장 필터 설정
        Filter postTypeFilter = MetadataFilterBuilder.metadataKey("postType").isEqualTo(postType.name());

//      3. 유사도 검색 실행
//      minScore를 통해 너무 관련 없는 결과는 제거하도록 함
//      필터 추가를 통해 해당하는 게시글 타입에서만 검색
        EmbeddingSearchRequest request = EmbeddingSearchRequest.builder()
                .queryEmbedding(queryEmbedding)
                .maxResults(maxResults)
...
//      3. 검색된 텍스트 조각들을 리스트로 변환
    ...
    @Override
    public List<TextSegment> searchByPostTypeWithGym(String query,Long gymId, PostType postType, int maxResults) {

//      1. 검색 질문 벡터로 변환 (임베딩 모델과 같은 모델을 사용해야한다.)
        Embedding queryEmbedding = embeddingModel.embed(query).content();
//      2. 암장 필터 설정
        Filter postTypeAndGymFilter = MetadataFilterBuilder.metadataKey("postType").isEqualTo(postType.name()).and(MetadataFilterBuilder.metadataKey("gymId").isEqualTo(gymId));

//      3. 유사도 검색 실행
//      minScore를 통해 너무 관련 없는 결과는 제거하도록 함
//      필터 추가를 통해 해당하는 게시글 타입과 암장 ID에 대해 검색
        EmbeddingSearchRequest request = EmbeddingSearchRequest.builder()
                .queryEmbedding(queryEmbedding)
                .maxResults(maxResults)
                .filter(postTypeAndGymFilter)
                .minScore(0.6) // 유사도 최소 0.6
                .build();
